(function () {
  ...
  function applyHeroPoster() {
    ...
    if (sobreImg && config.sobreImagem) {
      sobreImg.src = config.sobreImagem;
    }
  }
    ...
    grid.innerHTML = fotos
      .map((f) => {
        const ig = f.instagramUrl
          ...
          : '';
        return `
      <a href="${escapeHtml(f.imagem)}" class="gallery-item reveal" data-lightbox>
        <img src="${escapeHtml(f.imagem)}" alt="${escapeHtml(f.alt)}" loading="lazy" width="400" height="300" />
        ${ig}
      </a>`;
      })
      .join('');
  ...
  async function loadGaleriaFromJson() {
    ...
    try {
      const res = await fetch(config.instagramPostsJson);
      if (!res.ok) return;
  ...
  function applyLocalBusinessSchema() {
    ...
    try {
      ...
      if (config.ogImage) schema.image = config.ogImage;
